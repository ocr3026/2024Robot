...
public class AutoAim extends Command {
    ...
    public void execute() {
        ...
        if(target != null) {
            ...
            if(tagID == 3 || tagID == 7) {
                swerveSubsystem.drive(0, 0, -MathUtil.clamp(rotatePID.calculate(yaw, 0), -0.3, 0.3), false);
            ...
            if(tagID == 5 || tagID == 6) {
                swerveSubsystem.drive(0, 0, -MathUtil.clamp(rotatePID.calculate(zAngle, 180), -0.3, 0.3), false);

                if(zAngle <= 184 && zAngle >= 176) {
                    swerveSubsystem.drive(drivePID.calculate(yDist, 0), 0, 0, false);
